...
- **Репликация шардов**: Автоматическая репликация данных на несколько шардов для отказоустойчивости
...
│   │   └── replication_factor: u8 (коэффициент репликации)
│   └── MultiShardClient
...
Система поддерживает автоматическую репликацию данных на несколько шардов для обеспечения отказоустойчивости:

- **Настраиваемый коэффициент репликации**: Параметр `replication_factor` в конфигурации определяет количество реплик
- **Автоматическая запись на реплики**: При добавлении вектора система автоматически записывает его на основной шард и реплики
- **Чтение с реплик**: При недоступности основного шарда система автоматически читает данные с реплик
- **Детерминированный выбор реплик**: Реплики выбираются детерминированно для обеспечения согласованности
- **Согласованность данных**: `vector_id` генерируется на основе хеша содержимого, обеспечивая одинаковый ID на всех репликах
...
- **8083**: Шард 3 (только внутренний API)
- **8084**: Шард 4 (только внутренний API)
...
- **Шард 3**: `shard3_data` volume
- **Шард 4**: `shard4_data` volume

Все данные сохраняются в Docker volumes для персистентности. При репликации данные дублируются на нескольких шардах согласно `replication_factor`.
...
Система автоматически реплицирует данные при записи:
- При добавлении вектора он записывается на основной шард и реплики (согласно `replication_factor`)
- При чтении система проверяет все шарды и возвращает первый найденный вектор
- При обновлении/удалении операции выполняются на всех репликах
...
- **`replication_factor`**: Коэффициент репликации (по умолчанию: 2)
  - Определяет количество реплик для каждого вектора
  - Минимальное значение: 1 (без репликации)
  - Рекомендуемое значение: 2-3 для баланса между отказоустойчивостью и производительностью
  - При значении 2 каждый вектор будет храниться на основном шарде и одной реплике
    ...
    "sharding": {
        ...
        "shards": [
            ...
            },
            {
                ...
                "host": "vecdb-shard3",
                "port": 8083,
                "description": "Третий шард кластера"
            },
            {
                "id": "shard4",
                "host": "vecdb-shard4",
                "port": 8084,
                "description": "Четвертый шард кластера"
            }
        ]
    }
}
...
- ✅ **Отказоустойчивость** через автоматическую репликацию шардов
- ✅ **Репликация данных** с настраиваемым коэффициентом репликации
